...
export function ProjectsSeatMapPage() {
  ...
  useEffect(() => {
    if (selectedProject && gatePanelRef.current) {
      gatePanelRef.current.scrollIntoView({ behavior: "smooth", block: "nearest" });
    }
  }, [selectedProject]);
    ...
    <section className="py-10 sm:py-14 px-4 sm:px-6 lg:px-12">
      <div className="mx-auto max-w-[var(--runway-max)]">
        ...
        <div className="seat-map-container">
          <div className="airport-surface seat-map-surface rounded-xl border-2 border-[var(--floor-line)] p-4 min-h-[280px] flex justify-center">
            <SeatMap
              projects={projects}
          ...
          <div ref={gatePanelRef} className="gate-display-panel min-h-[280px] lg:min-h-0">
            {selectedProject ? (
